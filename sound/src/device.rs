...
use alsa_sys::{
    _snd_pcm_state_SND_PCM_STATE_DISCONNECTED, _snd_pcm_state_SND_PCM_STATE_DRAINING,
    _snd_pcm_state_SND_PCM_STATE_OPEN, _snd_pcm_state_SND_PCM_STATE_PAUSED,
    _snd_pcm_state_SND_PCM_STATE_PREPARED, _snd_pcm_state_SND_PCM_STATE_RUNNING,
    _snd_pcm_state_SND_PCM_STATE_SETUP, _snd_pcm_state_SND_PCM_STATE_SUSPENDED,
    _snd_pcm_state_SND_PCM_STATE_XRUN, _snd_pcm_stream_SND_PCM_STREAM_PLAYBACK,
    snd_pcm_bytes_to_frames, snd_pcm_close, snd_pcm_drain, snd_pcm_drop, snd_pcm_frames_to_bytes,
    snd_pcm_hw_params, snd_pcm_open, snd_pcm_pause, snd_pcm_prepare, snd_pcm_recover,
    snd_pcm_reset, snd_pcm_resume, snd_pcm_start, snd_pcm_state, snd_pcm_state_t, snd_pcm_t,
    snd_pcm_writei, EPIPE, ESTRPIPE,
};
...
impl Device {
    ...
    pub fn open<T: AsRef<str>>(name: T) -> AlsaResult<Self> {
        ...
        unsafe {
            snd_pcm_open(&mut snd_pcm, name.as_ptr(), _snd_pcm_stream_SND_PCM_STREAM_PLAYBACK, 0)
                .to_alsa_result()?;
        }
        Ok(Self { snd_pcm })
    }
    ...
    pub fn hw_params(&mut self, hw_params: &mut HwParams) -> AlsaResult<()> {
        unsafe { snd_pcm_hw_params(self.as_raw(), hw_params.as_raw()).to_alsa_result()? };
        Ok(())
    }
    ...
    pub fn play_wav<T: Read + Seek>(
        ...
        if wav.read_type(reader)?.as_str() != "WAVE" {
            return Err(io::Error::new(io::ErrorKind::Other, "RIFF file type is not WAVE"));
        }
        ...
        let header = wav
            ...
            .transpose()?
            .ok_or_else(|| {
                io::Error::new(io::ErrorKind::Other, "RIFF data is missing the \"fmt \" chunk")
            })?;
        ...
        if audio_format != WAV_FORMAT_PCM && audio_format != WAV_FORMAT_EXTENSIBLE {
            return Err(io::Error::new(io::ErrorKind::Other, "WAV is not in PCM format"));
        }
        hw_params.any(self).map_err(alsa_to_io_error)?;
        hw_params.set_access(self, Access::RwInterleaved).map_err(alsa_to_io_error)?;
        hw_params.set_channels(self, channel_count.into()).map_err(alsa_to_io_error)?;
        hw_params.set_rate(self, sampling_rate).map_err(alsa_to_io_error)?;
        ...
        hw_params.set_format(self, format).map_err(alsa_to_io_error)?;

        while let Err(err) = self.hw_params(hw_params).map_err(alsa_to_io_error) {
            const RETRY_TIMEOUT_US: u64 = 1000;
            log::error!("Error setting hw_params: {}. Retrying in {} us...", err, RETRY_TIMEOUT_US);
        ...
        let (offset, len) = wav
            ...
            .ok_or_else(|| {
                io::Error::new(io::ErrorKind::Other, "RIFF data is missing the \"data\" chunk")
            })?;
        ...
        match format {
            Format::S16Le => {
                ...
                Ok(Duration::from_secs_f64(
                    f64::from(len) / (2.0 * f64::from(sampling_rate) * f64::from(channel_count)),
                ))
            }
            Format::S32Le => {
                i32::volume_adjusted_copy(reader, self, volume)?;
                Ok(Duration::from_secs_f64(
                    f64::from(len) / (4.0 * f64::from(sampling_rate) * f64::from(channel_count)),
                ))
            }
...
impl Drop for Device {
    fn drop(&mut self) {
        unsafe {
            snd_pcm_close(self.as_raw()).to_alsa_result().expect("couldn't close ALSA device");
        }
    }
}
    ...
    fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
        loop {
            unsafe {
                ...
                let written = snd_pcm_writei(self.as_raw(), buf.as_ptr().cast(), frames as _);
                if written == -i64::from(EPIPE) || written == -i64::from(ESTRPIPE) {
                    log::error!("audio buffer underrun occurred");
...
impl From<snd_pcm_state_t> for State {
    fn from(state: snd_pcm_state_t) -> Self {
        match state {
            state if state == _snd_pcm_state_SND_PCM_STATE_OPEN => Self::Open,
            state if state == _snd_pcm_state_SND_PCM_STATE_SETUP => Self::Setup,
            state if state == _snd_pcm_state_SND_PCM_STATE_PREPARED => Self::Prepared,
            state if state == _snd_pcm_state_SND_PCM_STATE_RUNNING => Self::Running,
            state if state == _snd_pcm_state_SND_PCM_STATE_XRUN => Self::Xrun,
            state if state == _snd_pcm_state_SND_PCM_STATE_DRAINING => Self::Draining,
            state if state == _snd_pcm_state_SND_PCM_STATE_PAUSED => Self::Paused,
            state if state == _snd_pcm_state_SND_PCM_STATE_SUSPENDED => Self::Suspended,
            state if state == _snd_pcm_state_SND_PCM_STATE_DISCONNECTED => Self::Disconnected,

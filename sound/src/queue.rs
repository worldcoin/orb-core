...
impl Queue {
    ...
    pub fn spawn(
        card_name: &str,
        mut master_volume: impl FnMut() -> f64 + Send + 'static,
    ) -> io::Result<Self> {
            ...
            .spawn(move || {
                ...
                loop {
                    match queue_loop(
                        ...
                        Ok(()) => break,
                        Err(err) => log::error!("sound thread exited with error: {}", err),
                    }
                }
            })
            .expect("failed to spawn thread");
        Ok(Self { buffer, queue_event, cancel_event, counter, thread: Some(thread) })
    }
    ...
    fn push(&self, sound: Sound, cancel_all: bool) -> io::Result<()> {
        {
            ...
            let i = buffer.binary_search_by_key(&sound.id, |sound| sound.id).unwrap_err();
            buffer.insert(i, sound);
        }
        unsafe {
            let arg: u64 = 1;
            write(self.queue_event, ptr::addr_of!(arg).cast::<c_void>(), mem::size_of_val(&arg))?;
        }
        Ok(())
    }
}
...
impl SoundBuilder<'_> {
    ...
    pub fn push(self) -> io::Result<SoundFuture> {
        let Self { queue, name, reader, cancel_all, priority, max_delay, volume } = self;
        ...
        assert!(counter != 0, "the queue has played more than 10^16 sounds, how is this possible?");
        ...
        let sound = Sound { id, name, reader, deadline, state: Arc::clone(&state), volume };
...
fn queue_loop(
    ...
    queue_event: c_int,
    cancel_event: c_int,
    master_volume: &mut (impl FnMut() -> f64 + Send + 'static),
) -> io::Result<()> {
    ...
    loop {
        ...
        if let Some(mut sound) = sound {
            ...
            if play {
                let volume = master_volume() * sound.volume;
                log::info!("Playing sound {} with volume {}", sound.name, volume);
                // Reset any previously set cancel event.
                cancellable_sleep(Duration::ZERO, cancel_event)?;
                let start = Instant::now();
                let mut duration = device.play_wav(&mut sound, &mut hw_params, volume)?;
                ...
                if cancelled {
                    log::info!("Sound {} cancelled", sound.name);
            ...
            *state = State::Done(play);
            ...
            unsafe {
                let mut arg: u64 = 0;
                read(queue_event, ptr::addr_of_mut!(arg).cast::<c_void>(), mem::size_of_val(&arg))?;
            }
        }
    }
}
...
fn cancellable_sleep(dur: Duration, event: c_int) -> io::Result<bool> {
    ...
    unsafe {
        ...
        let n = select(event + 1, &mut fd_set, ptr::null_mut(), ptr::null_mut(), &mut tv)?;
        let cancelled = n > 0 && FD_ISSET(event, &fd_set);
        if cancelled {
            let mut arg: u64 = 0;
            read(event, ptr::addr_of_mut!(arg).cast::<c_void>(), mem::size_of_val(&arg))?;
        }
        Ok(cancelled)
    }
}

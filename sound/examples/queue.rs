...
async fn main() -> Result<()> {
    let queue = Queue::spawn("default", || 1.0)?;

    let connected = Sound(Arc::new(fs::read("sound/examples/voice_connected.wav")?));
    let overheating = Sound(Arc::new(fs::read("sound/examples/voice_overheating.wav")?));

    // Said firstly because the queue starts playing immediately.
    queue.sound(Some(Cursor::new(connected.clone())), "connected".to_string()).push()?;
    // Said thirdly because it's pre-empted by the next sound.
    queue
        .sound(Some(Cursor::new(connected.clone())), "connected".to_string())
        .volume(0.3)
        .push()?;
    ...
    // the queue.
    queue
        .sound(Some(Cursor::new(overheating.clone())), "overheating".to_string())
        .priority(1)
        .push()?;

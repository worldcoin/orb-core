...
{
    /// An intermediate input messages buffer size for cross-process communication.
    const SERIALIZED_INPUT_SIZE: usize;

    /// An intermediate output messages buffer size for cross-process communication.
    const SERIALIZED_OUTPUT_SIZE: usize;

    /// Agents have a configuration. This configuration is serialized and sent to the agent during its init. This const
    /// configures the expected size of the port buffer for serialized configuration. Some fields in the agent might be
    /// dynamically allocated, like a string.
...
impl<T: Port> Outer<T> {
    ...
    pub async fn send_unjam(&mut self, message: Input<T>) -> Result<()> {
        let mut send = self.tx.send(message).fuse();
        let mut recv = self.rx.next_ok();
        loop {
            select_biased! {
                result = send => return result.map_err(Into::into),
                result = recv => result.map(drop)?,
            }
        }
    }
}

impl<T: Port> SenderExt<Output<T>> for Inner<T> {
    fn send_now(&mut self, message: Output<T>) -> Result<()> {
        self.tx.send_now(message)
    }
}

impl<T: Port> SenderExt<Input<T>> for Outer<T> {
    fn send_now(&mut self, message: Input<T>) -> Result<()> {
        self.tx.send_now(message)
    }
}

impl<T: Port> ReceiverExt<Output<T>> for Outer<T> {
    fn try_recv(&mut self) -> Result<Option<Output<T>>> {
        self.rx.try_recv()
    }
}
    ...
    <T::Output as Archive>::Archived: Deserialize<T::Output, SharedDeserializeMap>,
{
    ...
    unsafe fn create(name: &str) -> Result<(*mut Self, OwnedFd)> {
        let size = Self::size_of();
        let raw_fd = memfd_create(&CString::new(name)?, MemFdCreateFlag::empty())? as RawFd;
        let fd = unsafe { OwnedFd::from_raw_fd(raw_fd) };
        ftruncate(
            fd.as_raw_fd(),
            size.get().try_into().expect("shared memory size to not be extremely large"),
        )?;
        let ptr = unsafe {
            mmap(
                None,
    ...
    unsafe fn from_fd(fd: OwnedFd) -> Result<*mut Self> {
        let ptr = unsafe {
            mmap(
                None,
                Self::size_of(),
    ...
    unsafe fn destroy(ptr: *mut Self) -> Result<()> {
        unsafe {
        ...
        unsafe {
            slice::from_raw_parts_mut(
                ...
                mem::size_of::<usize>()
                    ...
                    + T::SERIALIZED_CONFIG_EXTRA_SIZE,
            )
        }
    }
    ...
    <T::Output as Archive>::Archived: Deserialize<T::Output, SharedDeserializeMap>,
{
    ...
    ) -> Result<(OwnedFd, impl Future<Output = Result<(Self, InitialInputs)>>)> {
        let Self { tx, rx } = self;
    ...
    <T::Output as Archive>::Archived: Deserialize<T::Output, SharedDeserializeMap>,
{
    ...
    pub fn from_shared_memory(shmem_fd: OwnedFd) -> Result<Self> {
        Ok(RemoteInner {

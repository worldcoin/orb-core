    ...
    fn animate(&mut self, frame: &mut RingFrame, dt: f64, idle: bool) -> AnimationState {
        ...
        match self.shape.transition {
            Transition::Shrink => {
                ...
                self.shape.arc_max = (self.shape.arc_max - dt * ARC_GAP / 1.0).max(limit);
...
impl Shape {
    #[allow(clippy::cast_precision_loss)]
    pub fn render(&self, frame: &mut RingFrame) {
        ...
        arc =
            self.arc_min + arc * (self.arc_max - self.arc_min) / (PI * 2.0 / self.arc_count as f64);

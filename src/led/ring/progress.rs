...
impl Progress {
    ...
    #[must_use]
    pub fn new(initial_progress: f64, progress_duration: Option<f64>, color: Rgb) -> Self {
        Self {
            color,
            progress: initial_progress,
            progress_duration,
            pulse_angle: PULSE_ANGLE_RAD,
            shape: Shape { progress: 0.0, phase: 0.0, pulse_angle: PULSE_ANGLE_RAD },
        }
    }
...
impl Animation for Progress {
    ...
    fn animate(&mut self, frame: &mut RingFrame, dt: f64, idle: bool) -> AnimationState {
        ...
        self.shape.pulse_angle =
            self.shape.pulse_angle + (dt / (RC + dt)) * (self.pulse_angle - self.shape.pulse_angle);

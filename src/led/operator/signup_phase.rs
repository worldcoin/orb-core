...
use crate::{
    led::AnimationState,
    mcu::main::{OperatorLedsSequence, Rgb},
};
...
impl Animation for SignupPhase {
    ...
    fn animate(&mut self, frame: &mut OperatorLedsSequence, dt: f64, idle: bool) -> AnimationState {
        ...
        if !idle && !matches!(self.phase, Phase::Idle) {
            ...
            if self.warning_pulse_ph_rad > 0.0 && self.color[progress as usize % 5] != Rgb::OFF {
                ...
                // to animate it
                if let Some((i, _)) =
                    self.color.iter().enumerate().rev().filter(|(_, &c)| c != Rgb::OFF).last()
                {
                    animated_frame[i] = Rgb::OPERATOR_AMBER * self.warning_pulse_ph_rad.sin();
                }

use super::Animation;
use crate::{
    led::{operator::compute_smooth_blink_color_multiplier, AnimationState, Rgb},
...
impl Default for Battery {
    fn default() -> Self {
        Self { percentage: 100, is_charging: false, phase: 0.0 }
    }
}
...
impl Animation for Battery {
    ...
    fn animate(&mut self, frame: &mut OperatorLedsSequence, dt: f64, idle: bool) -> AnimationState {
        ...
        let multiplier =
            if blink { compute_smooth_blink_color_multiplier(&mut self.phase, dt) } else { 1.0 };
        if !idle {

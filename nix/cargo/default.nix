...
let
    ...
    runCommandLocal "${cargoToml.package.name}-${dep.name}-${cargoToml.package.version}-unpacked"
      {
        ...
        nativeBuildInputs = [ rustToolchain yq jq normalizeManifest ];
      }
      ''
        while read -r toml; do
          ...
          rm $dest/Cargo.toml
          cd $(cd $(dirname $toml); cargo metadata --no-deps --format-version 1 | jq -r .workspace_root)
          normalize-manifest < $toml > $dest/Cargo.toml
          exit 0
  ...
  cargoDerivation = { nameSuffix ? null, cargoArgs ? [ ], postBuildFlavour ? null, ... } @ args: stdenv.mkDerivation ({
    ...
    configurePhase = ''
      ...
      ln -s $cargoConfig $CARGO_HOME/config.toml
    ...
    buildPhase = ''
      ...
      for tuple in $flavours; do
        ...
        (set -x; $cmd)
        # On the next flavour run, the target directory may contain some
        # read-only files, which may be tried to be overwritten.
        chmod u+w -R target
  ...
  dummyCargoTomls = [ (dummyCargoToml cargoToml.package.name ".") ] ++ (map
    ...
    cargoToml.workspace.members or [ ]
  );
